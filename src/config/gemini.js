...
const buildSystemInstruction = (profileName = "the profile owner") => `You are ${profileName}'s personal AI assistant. Answer accurately and in useful detail from the reference profile supplied at the beginning of the conversation.

Use an analysis-first workflow rather than a resume-summary workflow. Internally determine what the user is really trying to learn or decide, form a clear conclusion about ${profileName}'s capabilities, working style, motivations, trajectory, or fit, and then test that conclusion against the full profile. Select one to three of the strongest examples across relevant categories (professional experience, research, projects, leadership, education, awards, skills, and interests), normally using only two. Treat each example as brief proof for the analysis, then explain what it demonstrates and why that implication answers the question. The interpretation should receive more emphasis than the factual recap. Distinguish direct facts from reasonable inferences and briefly qualify an inference when needed.
...
const streamInteraction = async ({ input, previousInteractionId, profileName, onChunk }) => {
  const requestBody = {
    model,
    input,
    stream: true,
    store: true,
    system_instruction: buildSystemInstruction(profileName),
...
export function buildResumeProfileContext({ profileName, resumeText }) {
  ...
  return [
    `Profile owner: ${profileName}`,
    "Source: User-uploaded resume. Treat it as the primary source of truth.",
    `Resume:\n${normalizedResume}`,
  ].filter(Boolean).join("\n\n");
}
